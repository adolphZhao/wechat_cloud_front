...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === '/interface-settings') {
          dispatch({
            type: 'query',
  ...
  effects: {
    ...
    * delete ({ payload }, { call, put, select }) {
      ...
      const { selectedRowKeys } = yield select(_ => _.interfaceSettings)
    ...
    * update ({ payload }, { select, call, put }) {
      const id = yield select(({ interfaceSettings }) => interfaceSettings.currentItem.id)
